...
Write-Host "TotalSegmentator GPU版 (ポータブル)" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
...
Write-Host "（処理には数分かかります）" -ForegroundColor Yellow
Write-Host ""

docker run --rm --gpus all -e HTTP_PROXY=http://gw.east.ncc.go.jp:8080 -e HTTPS_PROXY=http://gw.east.ncc.go.jp:8080 -v "${inputDir}:/input:ro" -v "${outputDir}:/output" totalsegmentor-app-gpu /input /output nifti false
